...
class Karger:
    ...
    def run(self):
        ...
        lista_aux = [adj_lista.copy() for adj_lista in self.lista_adj]  # Criando uma cópia profunda da lista de adjacência

        while len([lst for lst in lista_aux if lst]) > 2: # Se só restarem 2 vértices não vazios finaliza o loop
            aresta_escolhida = sample(lista_aresta_copy, 1)[0] # Escolhendo uma aresta aleatoriamente

            for elemento in (lista_aux[aresta_escolhida[1] - 1]):
                # Adicionando na lista de adjacencia do primeiro elemento todos os vertices do segundo elemento tirando ele mesmo
                if elemento not in lista_aux[aresta_escolhida[0] - 1] and elemento != aresta_escolhida[0]:
                    lista_aux[aresta_escolhida[0] - 1].append(elemento)
            ...
            lista_aux[aresta_escolhida[1] - 1].clear()
